  ...
  $(function(){
    ...
    window.input = function(it){
      ...
      return GET("a/" + it + ".json", function(json){
        var nodes, i$, ref$, len$, labels, $li, j$, len1$, ref1$, label, score, scene, camera, renderer, multiplier, objs, obj_coloring, obj_radius, clustering, i, cluster, c, l, n, coords, sphere_geo, mat, sphere, spritey, pos, edges, edge, color, sv, tv, geometry, line, controls, render, vertex, distance, material, particles;
    ...
    $(window).resize(function(){
      ...
      if ((ref1$ = window.camera) != null) {
        ref1$.aspect = window.innerWidth / window.innerHeight;
      }
      return (ref2$ = window.camera) != null ? ref2$.updateProjectionMatrix() : void 8;
    });
    ...
    window.makeTextSprite = function(message, arg$, parameters){
      var r, g, b, fontface, fontsize, borderThickness, borderColor, backgroundColor, canvas, context, metrics, textWidth, offsetX, offsetY, texture, spriteMaterial, sprite;
    ...
    if (/[^?]/.exec(location.search + "")) {
      return window.input((replace$.call(decodeURIComponent(location.search), /\?/, '')) + "");
    } else {
